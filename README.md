...
产物 `gomoku_engine` 是一个**服务端**，默认监听 `127.0.0.1:8888`。界面不需要
你去启动它：`engine.py` 会惰性连接，连不上就自己拉起，退出时经 `atexit` 停掉。
端口固定而非随机，是为了让"上一次运行残留的服务端还在跑"这件事能被
"先连一下试试"检测出来 —— 随机端口会让每次启动都留下一个孤儿进程。
...
对不上就**宁可不连**（退回本地引擎）。这是全链路里唯一"宁可降级也不能将就"的
检查 —— 接错程序时分值刻度错了会污染图表，比慢一点糟得多。
...
拿到的是**同一个模块对象** —— 于是两边都去 8888 上 spawn。第二个服务端 bind
...
注入三行）。修完实测两个服务端分别在 8901 / 8902 上监听。修后重跑 1.5 s 的
...
库里也不存这个局面。同样地，对手第一手落在**标准开局集之外**（比如角上）
...
- **服务端规矩是"最后连上的客户端胜出"**，两个客户端不能同时在 8888 上。
  游戏开着时建库会把游戏那条连接顶掉，游戏会**静默降级**到 Python 本地
  实现。所以建库默认自己另起一个端口（`--port`）。
...
这类用例标记为 `perf`，CI 跳过（`-m "not perf"`，**307 条**），本地跑全量
（**328 条**）。**正确性、增量一致性、时间合规、
...
| `test_opening_book.py` | 开局库的**接线**（不是表的内容）：命中即返回且 `nodes==0`、分值原样上报、键按走子方分区、**非标准开局必须回落搜索**（守降智陷阱）|
