...
// ==================== 被判死刑时不许认命 ====================
// 迭代加深原本在**任何**杀棋结论上收工，不分敌我。可 `isMate(val)` 对"我赢了"
// 与"我被将死"同时成立，于是被证明必败时搜索当场停在第 1 层 —— 实测退出时
// `dep=1`、耗时 0～86 ms，而那一手预留的预算是 5951 ms。剩下的算力全被扔掉，
// 返回的着法由排序（TT 着法 / 杀手 / 历史）决定，与局面无关，所以会往角上点
// A1 这种看着像程序坏掉的棋。
//
// 现在只对"我赢了"收工。被将死的那一侧继续加深：层数更高、窗口也更宽，一来
// 有可能找到浅层没看到的解法，二来**即使找不到**，各着法的杀棋距离也会在
// 同一个深度下算出来 —— 而杀棋分随距离单调（`WIN_SCORE - ply`），于是
// `v > best` 这条严格不等就能挑出被杀最晚的那一手。旧版全是同一个浅层的杀棋
// 分，并列之后只能靠排序瞎挑。
//
// 加多少由"**普通对弈的深度**"决定，而不是配置里的 `maxDepth`（那只是上限：
// 中级配置 10、实测 5）。见 `Engine::normalDepth_`。

//: 目标：比普通深度**深一层**。
constexpr int ESCAPE_EXTRA = 1;

//: 硬顶：深出来的层数**严格小于**这个数。
//:
//: 这条约束的形式来自需求原话 ——"最高不能超过普通 dep + 2，例如中级普通
//: dep=5，那么全力寻找出路的 dep 要满足 3 < dep < 7"。也就是目标必须落在
//: 开区间 `(普通-2, 普通+2)` 内，两端都不取：下界由 `ESCAPE_EXTRA > -2` 自动
//: 满足，上界靠下面这句 `static_assert` 钉住。
constexpr int ESCAPE_CAP = 2;
static_assert(ESCAPE_EXTRA < ESCAPE_CAP && ESCAPE_EXTRA > -ESCAPE_CAP,
              "全力找出路的深度必须落在 (普通-2, 普通+2) 开区间内");

//: 本档还没搜过一次正常局面时的保守估计（例如开局头几手就出事）。
//: 刻意取小：配置的 `maxDepth` 在中级是 10，拿它当"普通深度"会给出 11 层的
//: 目标，与"最多 +2"直接矛盾。
constexpr int ESCAPE_FALLBACK_NORMAL = 4;
